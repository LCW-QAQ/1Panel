...
var AddTableSetting = &gormigrate.Migration{
	...
	Migrate: func(tx *gorm.DB) error {
		...
		if err := tx.Create(&model.Setting{Key: "PanelName", Value: "ZPanel"}).Error; err != nil {
			return err
		}
...
var AddEntranceAndSSL = &gormigrate.Migration{
	...
	Migrate: func(tx *gorm.DB) error {
		if err := tx.Model(&model.Setting{}).
			Where("key_ = ? AND value = ?", "SecurityEntrance", "onepanel").
...
var EncryptHostPassword = &gormigrate.Migration{
	...
	Migrate: func(tx *gorm.DB) error {
		...
		if err := tx.Where("key_ = ?", "EncryptKey").Find(&encryptSetting).Error; err != nil {
			return err
		}
...
var UpdateRedisParam = &gormigrate.Migration{
	...
	Migrate: func(tx *gorm.DB) error {
		...
		if err := tx.Where("key_ = ?", "redis").First(&app).Error; err != nil {
			return nil
		}
