...
func loadParams(param string) string {
	stdout, err := cmd.Execf("grep '^%s=' /usr/local/bin/zpctl | cut -d'=' -f2-", param)
	if err != nil {
		panic(err)
	}
	info := strings.ReplaceAll(stdout, "\n", "")
	if len(info) == 0 || info == `""` {
		panic(fmt.Sprintf("error `%s` find in /usr/local/bin/zpctl", param))
	}
...
func loadChangeInfo() string {
	stdout, err := cmd.Exec("grep '^CHANGE_USER_INFO=' /usr/local/bin/zpctl | cut -d'=' -f2")
	if err != nil {
		return ""
	}
